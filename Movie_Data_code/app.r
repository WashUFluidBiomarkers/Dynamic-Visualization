...
ui <- shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(      selectInput("graphtype_anim","Type of Graph:",
                                   choices=c("Raindrop","Timetrails"),selected = "Timetrails"),
                       ...
                       checkboxInput("origin_plot", "Indicate baseline value of longitudinal variable? (Raindrop only)", 
                       ...
                       conditionalPanel("input.graphtype_anim=='Raindrop'",
                                        numericInput("th2_rank", 
                                                     label = "X-axis threshold line (rank)",
                                                     value=90)),
                       conditionalPanel("input.graphtype_anim=='Timetrails'",
                                        numericInput("th2_trail",  #threshold 2 field - x-axis
                                                     label = "X-axis threshold line",
                                                     value=0.158,step = 0.0005)),
                       numericInput("th1", 
                                    label = "Y-axis threshold line (value)",
                                    value=0.0673,step = 0.0005),
                       tags$hr(),
                       div(style="display: inline-block; width: 125px;",
                           sliderInput("cutoff1","Group cut-off 1",min=0,max=20,
                                       step = 0.2,value=0,width='100%')),
                       div(style="display: inline-block; width: 125px;",
                           sliderInput("cutoff2","Group cut-off 2",min=0,max=20,
                                       step = 0.2,value=0,width='100%')),
                       div(style="display: inline-block; width: 125px;",
                           sliderInput("cutoff3","Group cut-off 3",min=0,max=20,
                                       step = 0.2,value=0,width='100%')),
                       div(style="display: inline-block; width: 125px;",
                           sliderInput("cutoff4","Group cut-off 4",min=0,max=20,
                                       step = 0.2,value=0,width='100%')),
    ...
    mainPanel(
      #   sliderInput("cutoffs","Group cut-offs",min=0,max=20,
      #                step = 0.2,value=c(1,4),width='100%'),
      
      img(src='double_logo_v1.png', align = "bottom",  width = 1200, height = 250)
    )
  )
)
#Dynamic Color categories
#Make able to take plasma, APS, PET, ETC. - Will receive other data
#Defaults CSF 0.0673 / 90, Plasma 0.1013 / Rank 144, APS - 15/Rank 274, Centiloid 16 / Rank 208
)
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if(!is.null(query[['th1']])){
      ...
      updateNumericInput(session,'th1',
                         value=th1)
    }
    
    if(!is.null(query[['th2']])){
      th2 <- as.numeric(query[['th2']])
      updateNumericInput(session,'th2',
                         value=th2)
  ...
  variableids_for_plot <- reactive({
    if(input$graphtype_anim=="Raindrop"){
      ...
      graph_choice_y <- graph_choices_suffixes[which(graph_choices_ref==input$y_var)] #uses main variable label vector to determine which variable name to pull - y variable
      graph_choice_x <- graph_choices_suffixes[which(graph_choices_ref==input$x_var)] #uses main variable label vector to determine which variable name to pull - x variable
      graph_vars_list <- c(paste0("EST_",graph_choice_x), #compile x-variable id
                           paste0("EST_",graph_choice_y)) #compile y-variable id
    }
    graph_vars_list #prints to output
  })
  
  param_change <- reactive({list(input$graphtype_anim,input$cutoff1,input$cutoff2,input$cutoff3,input$cutoff4,
                                 input$y_var,
                                 input$x_var,
  ...
  cutoff_change <- reactive({list(input$group_xy_check,
                                  input$other_type,
                                  input$otherb_type,
                                  input$reset_thresholds)})
  ...
  observeEvent(c1_change(),{
    min2 <- input$cutoff1
    updateSliderInput(session, "cutoff2",
                      min = min2)
  })
  
  observeEvent(c2_change(),{
    min3 <- input$cutoff2
    updateSliderInput(session, "cutoff3",
                      min = min3)
  })
  observeEvent(c3_change(),{
    min4 <- input$cutoff3
    updateSliderInput(session, "cutoff4",
                      min = min4)
  })
  ...
  observe({
    ...
    if(input$graphtype_anim=="Raindrop"){
      file_name_out <- paste0("out_raindrop_",yvar_filename,"_vs_",xvar_filename,".mp4")
    }
    if(input$graphtype_anim=="Timetrails"){
      file_name_out <- paste0("out_timetrails_",yvar_filename,"_vs_",xvar_filename,".mp4")
    }
    ...
    output$downloadData <- downloadHandler(
      ...
      content = function(file) {
        ...
        xvar_for_vid <- graph_choices_suffixes[which(graph_choices_ref==input$x_var)]
        yvar_for_vid <- graph_choices_suffixes[which(graph_choices_ref==input$y_var)]
        ogn <- update_origin_point()
        ...
        if(input$graphtype_anim=="Raindrop"){
          ...
          #create fixed rank limits vector - check CDR/Tau PET/Cortical Thickness variables specifically for stability
          # Stability is most improtant
          #finish matrix of rank guidelines
          ...
          withProgress(message = 'Video being created', 
                       ...
                       value = 0, {
                         av_capture_graphics(create_raindrop_plot_movie(data = in.csv.graphdata,
                                                                        ...
                                                                        th1=input$th1,th2=input$th2_rank,
                                                                        originpoints=FALSE),
                                             output = file,
                                             res=100,
                                             width=1250,
          ...
          withProgress(message = 'Video being created', 
                       ...
                       value = 0, {
                         ...
                         av_capture_graphics(create_timepath_plot_video(data = in.csv.graphdata,
                                                                        ...
                                                                        graph.xlab = input$x_var,
                                                                        graph.ylab = input$y_var,
                                                                        th1=input$th1,th2=input$th2_trail),
                                             output = file,
                                             res=100,
                                             width=1250,
                                             height=750, 
  ...
  observeEvent(input$x_var,{
    ...
    updateNumericInput(session,'th2_rank',
                       value=updated_threshold_th2)
    if(!is.null(query[['th2']])){
      th2 <- as.numeric(query[['th2']])
      updateNumericInput(session,'th2_rank',
                         value=th2)
    ...
    updateNumericInput(session,'th2_trail',
                       value=updated_threshold_th2)
    if(!is.null(query[['th2']])){
      th2 <- as.numeric(query[['th2']])
      updateNumericInput(session,'th2_trail',
