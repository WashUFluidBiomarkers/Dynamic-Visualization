# Raindrops: Baseline/Rank Raindrop Viewer
# Tidyverse is critical for plotting and data filtering

# Use ragg for high-quality, fast PNG rendering in Shiny
options(shiny.useragg = TRUE)

# Install/load required packages
list.of.packages <- c(
  "tidyverse", "data.table", "shiny", "RColorBrewer", "ggforce",
  "ggbreak", "magick", "av", "ragg", "ggrastr", "cachem","conflicted","khroma"
)
...
# Step size per biomarker for sliders
step_size_list <- c(
  0.1, 0.2, 0.005, 0.001, 0.001, 0.005, 0.005, 0.01, 0.005, 0.001,
  0.05, 0.01, 1, 0.005, 0.01, 1, 0.01, 0.01, 0.001, 0.5, 1
)

# Selectable variable labels
graph_choices_ref <- c(
  paste0("C2N Precivity AD2 ","plasma %p-tau217 (%)"),
...
# Axis limits per variable
graph_limits_list <- list(
  c(0,25), c(0,20), c(0,0.15), c(0,1.8), c(0,0.3), c(0,3), c(0,0.35), 
  c(0,4.7), c(0,0.23), c(0,0.46), c(0,16), c(0,80), c(0,565), c(0,0.125), 
  c(0,80), c(-40,190), c(0.85,2), c(0.9,2.5), c(1.8,3.25), c(-0.5,10), c(5,185)
)

# Default group threshold strings for each variable
group_threshold_defaults <- c(
  "1.5, 2.5, 4.1, 8", "3, 7, 13, 16", "0.04, 0.06, 0.0924, 0.1",
  "0.06, 0.1, 0.158, 0.5", "0.04, 0.06, 0.0869, 1.5", "0.2,0.3,0.444,0.5",
  "0.04,0.05,0.0615,0.085", "0.65,1.0,1.14,1.3", "0.11,0.12,0.126,0.14",
  "0.0661, 0.0916, 0.113, 0.1230", "2.650, 3.270, 4.29, 7",
  "12.1, 17.8, 20.5, 30", "89, 128, 205, 250", "0.045, 0.053, 0.0582, 0.070",
  "13.27, 17.57, 21.7, 28", "-5.00, 7.00, 20, 39", "1.09, 1.15, 1.328, 1.4",
  "1.05, 1.12, 1.269, 1.31", "2.5, 2.572, 2.932, 3", "0.5, 1, 2, 4",
  "10,25,60,91"
)

# Rank cutoffs per variable for the plot
rank_thresholds <- c(
  236, 254, 186, 241, 160, 250, 252, 237, 209, 250, 
  269, 203, 260, 130, 217, 233, 103, 114, 26, 266, 53
)

# Default threshold lines for each variable
value_thresholds <- c(
  4.06, 2.34, 0.0924, 0.158, 0.0869, 0.444, 0.0615, 1.14, 0.126, 0.113,
  4.29, 20.5, 205, 0.0582, 21.7, 20, 1.328, 1.269, 2.572, 1, 25
)
  ...
  div(class = "custom-header",
      div(class = "header-content",
          div(class = "title-section",
              h1("Alzheimer's Disease Biomarker Raindrop Viewer", class = "app-title"),
              p("Longitudinal biomarker trajectories aligned by baseline rank", class = "subtitle")
          ),
  ...
  div(id = "analysis-section", class = "content-section active",
      sidebarLayout(
        sidebarPanel(
          ...
          checkboxInput("origin_plot", "Indicate baseline value of longitudinal variable?", value = TRUE, width = NULL),
          selectInput("x_rank", "X-axis variable (baseline rank):",
                      choices=graph_choices_ref[scramble_key],selected = graph_choices_ref[16]),
          tags$hr(),
          sliderInput("Time","Years",min=0,max=10, step = 0.2,value=2,
                      animate=animationOptions(
                        interval = 600,  
                        loop = FALSE,
                        playButton = actionButton("play",
                                                  HTML('<img src="https://findicons.com/files/icons/1056/flavours/48/button_play_red.png">'),
                                                  width=100),
                        pauseButton = actionButton("play",
                                                   HTML('<img src="https://findicons.com/files/icons/2192/flavour_extended/48/button_pause_red.png">'),
                                                   width=100))
          ),
          tags$hr(),
          ...
          conditionalPanel("input.group_xy_check =='Baseline'",
                           selectInput("otherb_type", "Base group colors off which value?",
                                       graph_choices_ref[scramble_key],selected = graph_choices_ref[1])),
          h6("Points without grouping variable will be grey"),
          numericInput("th2", label = "X-axis threshold line (rank)", value=90),
          numericInput("th1", label = "Y-axis threshold line (value)", value=2.34, step = 0.0005),
          tags$hr(),
          uiOutput("tab")
        ),
        mainPanel(
          ...
          fluidRow(
            column(12,
                   div(class = "references-section",
                       ...
                       #                       p("Models are trained on data from the Alzheimer's Disease Neuroimaging Initiative (ADNI) and other research cohorts, providing evidence-based #probability estimates for clinical research applications.")
                   )
            )
          )
      )
...
server <- function(input, output, session) {
  ...
  video_print_now <- reactive({list(input$video_out)})
  
  observe({
    query <- parseQueryString(session$clientData$url_search)
    if(!is.null(query[['xvar']])){
      updateSelectInput(session, "movieout", selected = "1")
    }
  })
  ...
  variableids_for_plot <- reactive({
    ...
    graph_choice_BL <- var_suffix_by_label[[input$x_rank]]
    c(
      paste0("N_",graph_choice_BL),
      paste0("BL_EST_",graph_choice_BL),
      paste0("EST_",graph_choice_y)
    )
  }) %>% bindCache(input$y_var, input$x_rank)
  
  param_change <- reactive({
    list(input$cutoff1,input$cutoff2,input$cutoff3,input$cutoff4,
         input$y_var,input$x_rank,input$group_xy_check,
         input$other_type,input$otherb_type,input$th1,input$th2,input$origin_plot)
  })
  
  cutoff_change <- reactive({
    list(input$group_xy_check, input$other_type,
         input$otherb_type, input$reset_thresholds)
  })
  ...
  observeEvent(cutoff_change(),{
    ...
    updateSliderInput(session, "cutoff1",value=group_thresholds[1],
                      min = r[["min"]], max = r[["max"]], step = var_step)
  ...
  data_cat <- reactive({
    ...
    bindCache(
      ...
      input$cutoff1, input$cutoff2, input$cutoff3, input$cutoff4,input$origin_plot
    ) %>%
    bindEvent(param_change())
  
  update_group_long <- reactive({list(input$group_xy_check,
                                      input$other_type,
                                      input$otherb_type)})
  update_origin_point <- reactive({input$origin_plot}) %>% bindCache(input$origin_plot) %>%
    bindEvent(param_change())
  ...
  output$plot1 <- renderCachedPlot({
    ...
    variables_for_plot <- variableids_for_plot()
    origin_point_var <- update_origin_point()
    ...
    create_raindrop_plot_pro(
      in.csv.graphdata,
      (input$Time - 1),
      input$Time,
      variables_for_plot, limits_for_plot,
      input$x_rank, input$y_var,
      input$th1, input$th2,originpoints = origin_point_var
    )
  },
  cache = "session",
  cacheKeyExpr = {
    list(
      x = input$x_rank, y = input$y_var,
  ...
  observeEvent(param_change(),{
    ...
    x_variable_id <- var_suffix_by_label[[input$x_rank]]
    ...
    extra_params <- thlines()
    origin_point_var <- update_origin_point()
    group_breaks <- group_cutoffs()
    group_thresholds <- extract_comma_sep(group_breaks)
    out_args_opt <- paste(paste0(leadingurl,"?",
                                 "graphtype=","Raindrop&",
                                 "xvar=",x_variable_id),
                          paste0("yvar=", y_variable_id),
                          paste0("gxyc=",extra_params[[3]]),
                          ...
                          paste0("th1=",extra_params[[1]]),
                          paste0("th2=",extra_params[[2]]),
                          paste0("ogn=",origin_point_var),
                          sep="&")
    url <- a("Generate MP4 in new tab", href=out_args_opt,target="_blank")
    output$tab <- renderUI({ tagList(url) })
  })
  
  observeEvent(input$x_rank,{
    x <- input$x_rank
