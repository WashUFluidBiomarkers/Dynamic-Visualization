...
graph_choices_ref <- c(paste0("C2N Precivity ","plasma %p-tau217 (%)"),
...
#The below function's purpose is to create a color coding index to be used by the plot functions
# This one specifically is to create a color coding where the lowest values are considered to be the most
# at risk. This is primarily used for amyloid beta ratios
...
#The below function's purpose is to create a color coding index to be used by the plot functions
# This one specifically is to create a color coding where the highest values are considered to be the most
# at risk. This is used for most markers of alzheimer's
...
# this is a function to create a vector of objects from a text string with commas
#this is used in the group color code creation process since we store the thresholds as strings
...
#This is one of the primary functions of the web page
...
numberline_cats <- function(thresholds,xminline,xmaxline,flip_flag){
  x=thresholds
  col_set <- c( "A","B","C","D")
  mycols <- c("A"="#dc143c", "B"="#ffa500", "C"="#3cb371", "D"="#4169e1", "E"="#4b0082","NODATA"="#D3D3D3")
  if(flip_flag==TRUE){
  ...
  ggplot(data.frame(x,col_set), aes(x=x, y=0)) +
    geom_point(size = 10)  +
    annotate("segment",x=xminline,xend=xmaxline, y=0, yend=0, size=2) +
    annotate("segment",x=xminline,xend=x[1], y=0, yend=0, size=2,color=mycols[1]) +
    annotate("segment",x=x[1],xend=x[2], y=0, yend=0, size=2,color=mycols[2]) +
    annotate("segment",x=x[2],xend=x[3], y=0, yend=0, size=2,color=mycols[3]) +
    annotate("segment",x=x[3],xend=x[4], y=0, yend=0, size=2,color=mycols[4]) +
    annotate("segment",x=x[4],xend=xmaxline, y=0, yend=0, size=2,color=mycols[5]) +
    annotate("segment",x=xminline,xend=xminline, y=-0.1,yend=0.1, size=2) +
    annotate("segment",x=xmaxline,xend=xmaxline, y=-0.1,yend=0.1, size=2) +
    geom_text(aes(label = x), col="white") +
    scale_x_continuous(limits = c(xminline,xmaxline)) +
...
create_timepath_plot <- function(data,
 ...
 data.1 <- data
  ...
  #Needs rewrite
  data.for.graph.latest <- data.for.graph %>% group_by(RID) %>% dplyr::filter(time==max(time) & time >= time2-0.2)
  endtime <- max(unique(data.for.graph.latest$time))
  data.for.graph.leadup <- data.for.graph %>% group_by(RID) %>% dplyr::filter(time!=max(time) & time < endtime)
  #Don't like that this is very dependent on interval
  data.for.graph.leadup.line <- data.for.graph %>% group_by(RID) %>% arrange(RID,time,by_group=TRUE) %>% slice(tail(row_number(), 2)) 
  N_subs <- nrow(unique(data.for.graph.latest[,"RID"]))
  Title_for_plot <- paste0("Years since baseline: ",format(round(time2,digits = 2),nsmall=1),", Number of individuals:", N_subs)
  ...
  if(graph.ylab == graph_choices_ref[1] | graph.ylab == graph_choices_ref[3] | graph.ylab == graph_choices_ref[8] | graph.ylab == graph_choices_ref[10]){
...
create_timepath_plotly <- function(data,
                                 time1,time2,
                                 variablelist,
                                 graph.xlab,graph.ylab,
                                 xlimits,ylimits,
...
create_timepath_plot_video <- function(data,
  ...
  graph_vars_list <- variablelist
  data.1 <- data[which(complete.cases(data[,graph_vars_list])),]
  ymax <- max(data.1[,graph_vars_list[2]]) + abs(mean(data.1[,graph_vars_list[2]])*0.01)
  xmax <- max(data.1[,graph_vars_list[1]]) + abs(mean(data.1[,graph_vars_list[1]])*0.01)
  xmin <- min(data.1[,graph_vars_list[1]])- abs(mean(data.1[,graph_vars_list[1]])*0.01)
  ymin <- min(data.1[,graph_vars_list[2]])- abs(mean(data.1[,graph_vars_list[2]])*0.01)
  
  
xreverse  <- (graph.xlab == graph_choices_ref[1] | graph.xlab == graph_choices_ref[3] | graph.xlab == graph_choices_ref[8] | graph.xlab == graph_choices_ref[10])
yreverse <- (graph.ylab == graph_choices_ref[1] | graph.ylab == graph_choices_ref[3] | graph.ylab == graph_choices_ref[8] | graph.ylab == graph_choices_ref[10])  
  if(graph.ylab == graph_choices_ref[6]){
    ymax <- 2000
  }
  if(graph.ylab == graph_choices_ref[7]){
    ymax <- 200
  }
  if(graph.xlab == graph_choices_ref[6]){
    xmax <- 2000
  }
  if(graph.xlab == graph_choices_ref[7]){
    xmax <- 200
  }
  for(time_set in 1:100){
    
    time2 <- (time_set-1)*0.2
    time1 <- time2-1
    ...
    endtime <- max(unique(data.for.graph.latest$TIME))
    data.for.graph.leadup <- data.for.graph %>% group_by(RID) %>% filter(TIME!=max(TIME) & TIME < endtime)
    data.for.graph.leadup.line <- data.for.graph %>% group_by(RID) %>% filter(TIME==max(TIME) | TIME == min(TIME)) %>% arrange(RID,TIME,by_group=TRUE)
    N_subs <- nrow(unique(data.for.graph.latest[,"RID"]))
    Title_for_plot <- paste0("Years since baseline: ",format(round(time2,digits = 2),nsmall=1),", Number of individuals:", N_subs)
    omnidir_plot  <- ggplot(data=data.for.graph.latest)+
      geom_path(data=data.for.graph.leadup.line,aes_string(x=graph_vars_list[1],y=graph_vars_list[2],group = "RID",color="groupvar"),
      ...
      theme(aspect.ratio = 0.5,legend.position = "none",
            ...
            axis.text.y = element_text(size=13),
            plot.title = element_text(size=14,face="bold",hjust=1),
            plot.margin = margin(0.5,0.5,1.0,1.0, "cm"))+
      ggtitle(Title_for_plot)
