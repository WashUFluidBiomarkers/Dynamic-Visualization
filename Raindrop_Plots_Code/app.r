  ...
  div(id = "analysis-section", class = "content-section active",
      sidebarLayout(
        sidebarPanel(
          ...
          checkboxInput("origin_plot", "Indicate baseline value of longitudinal variable?", value = FALSE, width = NULL),
...
server <- function(input, output, session) {
  ...
  observeEvent(param_change(), {
    ...
    graph_choice_BL <- graph_choices_suffixes[which(graph_choices_ref==input$x_rank)]
    oty_variable_id <- graph_choices_suffixes[which(graph_choices_ref==input$other_type)]
    otyb_variable_id <- graph_choices_suffixes[which(graph_choices_ref==input$otherb_type)]
    ...
    out_args_opt <- paste(
      paste0(leadingurl,"?","graphtype=","Raindrop"),
      paste0("xvar=",graph_choice_BL),
      paste0("yvar=", graph_choice_y),
